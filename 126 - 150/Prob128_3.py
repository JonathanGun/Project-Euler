...
for layer in range(1, layers + 1):
    x = sumfac(layer) * 6 + 1
    num[layer].append(x - layer * 6 + 1)
    num[layer].append(x - layer * 6 + 2)
    num[layer].append(x - 1)
    num[layer].append(x)
    num.append([])
    if layer % 1000 == 0:
        print("layer", layer, layer / layers * 100, "%")


# for n in range(2, limit + 1):
#     x = sumfac(layer) * 6 + 1
#     if n <= x:
#         if n == x - layer * 6 + 1 or n == x - layer * 6 + 2 or n == x - 1 or n == x:
#             num[layer].append(n)
#     else:
#         layer += 1
#         num.append([])
